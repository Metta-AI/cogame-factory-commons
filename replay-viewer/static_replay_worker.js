...
function stageNote() {
  ...
  try {
    var length = Module._factory_commons_stage_len ? Module._factory_commons_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._factory_commons_stage_ptr();
...
function runtimeError() {
  var length = Module._factory_commons_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._factory_commons_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function ingestPacket() {
  var length = Module._factory_commons_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._factory_commons_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._factory_commons_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._factory_commons_load_replay(pointer, length);
    });
    if (!loaded) throw new Error(runtimeError());
    runtimeLoaded = true;
    ingestPacket();
    // No mismatch tick: Factory Commons records STATE, not inputs, so
    // playback never re-simulates and there is nothing to mismatch.
...
function advance(frames) {
  ...
  try {
    ...
    for (var i = 0; i < count; i++) {
      if (Module._factory_commons_frame() < 0) throw new Error(runtimeError());
      ingestPacket();
    }
    postMessage({
      type: 'advanced',
...
importScripts('./wire_constants.js', './broadcast_core.js', './factory_commons_replay.js');
