(function () {
  ...
  function showFailure(error) {
    ...
    failed = true;
    console.error(error);
    // The signal tools/ci/viewer_smoke.mjs and phase 60's viewer-check.yml
    // read on failure. Without it a viewer that dies mid-load looks the same
    // to CI as one that is merely slow.
  ...
  function createCore(config) {
    ...
    function start() {
      ...
      try {
        worker = new Worker(workerUrl, { name: 'factory-commons-static-replay' });
