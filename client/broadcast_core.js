...
//
// FORKED FROM coworld-ctf (paintbot). Its ingest/packet plumbing, letterboxing,
// layer pooling and static-band cache are UNTOUCHED — they are the renderer,
// and the playbook's rule is to treat them as the exact template. What differs
// in Factory Commons is the BOARD ITSELF, and that is drawn server-side into
// sprite pixels (src/factory_commons/global.nim: a tiled steel floor, the wall
// ring, the 5x5 machine block in four art states, the console and its two
// levers, the maintenance bay, the two dispenser mouths and their belts with
// marching chevrons, the two hopper intakes, the chute, pink and blue cubes,
// bananas, and three cogs with the carried cube over the head) — so this file
// composites a different picture without a different line of code.
//
// One inherited path is deliberately INERT here: `attachMinimap` / `drawMinimap`
// only ever run when the page hands over a minimap surface, and Factory Commons
// has no #viewpanel (the 1248x720 board always fits the frame). The code stays
// so this file remains a readable diff against paintbot's.
