...
# `mkdir -p` the parent BEFORE resolving it: paintbot's hook resolves its
# output path by cd-ing into the parent, which `coworld build` pre-creates but
# a fresh CI checkout does not — so the unfixed hook exits 1 in ci.yml while
# working locally (ecos, 2026-08-23). Every fork of this hook inherits the bug.
...
image_tag="coworld-factory-commons-replay-viewer-build:$$"
container_id=""
cleanup() {
...
docker cp "${container_id}:/workspace/factory_commons/replay-viewer/dist/." "${output_dir}"
